...
class Test extends Request {
  ...
  serverAddress(app, path) {
    ...
    //   this._server = app;
    // }
    // Keep the client on the address family selected by the server. On macOS,
    // an IPv6 wildcard can share a port with a specific IPv4 listener.
    let host = addr.address;
    if (host === '::') host = '::1';
    else if (host === '0.0.0.0') host = '127.0.0.1';
    if (addr.family === 'IPv6' || addr.family === 6) host = `[${host}]`;
