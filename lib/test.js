...
class Test extends Request {
  ...
  _assertHeader(header, res) {
    ...
    // Use `res.headers` rather than `res.header`: superagent copies
    // content-type parameters onto the response, so a value such as
    // `text/csv; header=present` overwrites `res.header` and breaks every
    // header assertion. `res.headers` is the untouched header map.
