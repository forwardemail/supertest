...
class Test extends Request {
  ...
  serverAddress(app, path) {
    if (!app.address()) {
      app.listen(0);
      // The counter marks the server as supertest-started: only those servers
      // are closed, and only after the last in-flight request finishes, so
      // concurrent requests can share one server.
      app._supertestInflight = 0; // eslint-disable-line no-param-reassign
    }
    if (app._supertestInflight !== undefined) {
      app._supertestInflight += 1; // eslint-disable-line no-param-reassign
      this._server = app;
    }
   ...
   */
    ...
    super.end((err, res) => {
      ...
      // Release this test's slot exactly once even if end() runs twice,
      // e.g. .expect(status, fn) followed by await.
      if (server && !this._serverReleased) {
        this._serverReleased = true;
        server._supertestInflight -= 1;
      }

      if (server && server._supertestInflight === 0 && server._handle) {
        // Unmark before closing so a server the user re-listens on later is
        // not adopted (and closed) by supertest again.
        delete server._supertestInflight;
